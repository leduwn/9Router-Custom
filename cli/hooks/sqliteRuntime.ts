...
const CLI_ROOT = process.env.NINEROUTER_CLI_ROOT
...
function npmInstall(pkgs, opts: { optional?: boolean; silent?: boolean; timeout?: number } = {}) {
...
function isSqlJsWasmValid() {
  const bundledWasm = path.join(CLI_ROOT, "app", "node_modules", "sql.js", "dist", "sql-wasm.wasm");
...
function buildEnvWithRuntime(baseEnv = process.env) {
  ...
  const bundledNm = path.join(CLI_ROOT, "app", "node_modules");
