...
const cliDir = path.basename(path.resolve(__dirname, "..")) === "dist"
...
// Step 3a: Copy generated custom server (injects real socket IP, strips spoofable XFF).
const customServerSrc = path.join(appDir, ".runtime", "custom-server.js");
if (fs.existsSync(customServerSrc)) {
  ...
  console.warn("⚠️  generated custom-server.js not found — server will run without real-IP injection\n");
}
...
// Step 7: Copy generated runtime artifacts not bundled by Next.js standalone.
console.log("7️⃣  Copying runtime artifacts...");
const runtimeRoot = path.join(appDir, ".runtime");
const mitmSrc = path.join(runtimeRoot, "src", "mitm");
const mitmDest = path.join(cliAppDir, "src", "mitm");
if (fs.existsSync(mitmSrc)) {
  copyRecursive(mitmSrc, mitmDest);
  console.log("✅ Copied MITM runtime\n");
} else {
  console.log("⏭️  No MITM runtime found\n");
}

const updaterSrc = path.join(runtimeRoot, "src", "lib", "updater");
const updaterDest = path.join(cliAppDir, "src", "lib", "updater");
if (fs.existsSync(updaterSrc)) {
  copyRecursive(updaterSrc, updaterDest);
  console.log("✅ Copied updater runtime\n");
} else {
  console.log("⏭️  No updater runtime found\n");
}
