...
const EXTERNALS = [];
const ENTRIES = [{ source: "server.ts", output: "server.js" }];

async function buildEntry(entry) {
  const mitmSrc = path.join(appDir, "src", "mitm");
  const output = path.join(cliMitmDir, entry.output);
  ...
  if (BUILD_CONFIG.bundle) {
    await esbuild.build({
      entryPoints: [path.join(mitmSrc, entry.source)],
      bundle: true,
...
async function run() {
  ...
  if (BUILD_CONFIG.cleanPlainFiles) {
    const keep = new Set(ENTRIES.map((entry) => entry.output));
