...
function imageInputToUrl(input: string) {
...
async function pollUntilDone({ host, port, apiKey, requestId, connectionId, timeoutSec, pollIntervalMs, signal, onProgress }: {
  ...
  pollIntervalMs: number;
  signal?: AbortSignal;
  onProgress?: (status: string, progress: unknown) => void;
}) {
...
function gatewayRequestWithConnection({ host, port, apiKey, requestId, connectionId, signal }: {
  ...
  connectionId: string | string[] | null;
  signal?: AbortSignal;
}): Promise<Omit<GatewayResponse, "headers">> {
  return new Promise((resolve, reject) => {
    const headers: Record<string, string | string[]> = { Accept: "application/json" };
    if (apiKey) headers.Authorization = `Bearer ${apiKey}`;
 ...
 */
async function downloadToFile(url: string, outputPath: string, { signal }: { signal?: AbortSignal } = {}) {
