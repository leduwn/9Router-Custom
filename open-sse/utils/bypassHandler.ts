 ...
 */
function createNonStreamingResponse(sourceFormat, model, text = DEFAULT_BYPASS_TEXT) {
 ...
 */
function createStreamingResponse(sourceFormat, model, text = DEFAULT_BYPASS_TEXT) {
  const openaiResponse = createOpenAIResponse(model, text);
  const state = initState(sourceFormat);
  state.model = model;
