...
export function buildUsage({ promptTokens, completionTokens, totalTokens, cachedTokens = 0, cacheCreationTokens = 0, reasoningTokens = 0 }) {
  const usage: Record<string, any> = { prompt_tokens: promptTokens, completion_tokens: completionTokens, total_tokens: totalTokens };
...
const USAGE_EXTRACTORS = {
  ...
  kiro(raw) {
    ...
    const out: Record<string, any> = { promptTokens: input, completionTokens: output, totalTokens: input + output };
