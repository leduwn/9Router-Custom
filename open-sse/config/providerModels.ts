...
import { modelQuotaFamily, modelStrip, modelTargetFormat, normalizeModelId } from "../providers/models/schema";
...
export const OAUTH_ALIASES = Object.fromEntries(
  (REGISTRY as Array<Record<string, any>>).filter(r => r.alias && r.alias !== r.id).map(r => [r.id, r.alias])
);
