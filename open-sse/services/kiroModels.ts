...
async function fetchKiroCatalogRaw(credentials, signal) {
  ...
  if (!response.ok) {
    ...
    const err: any = new Error(`Kiro ListAvailableModels ${response.status}: ${text || response.statusText}`);
 ...
 */
export async function resolveKiroModels(credentials, options: Record<string, any> = {}) {
