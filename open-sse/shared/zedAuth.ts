...
const llmTokenCache = new Map<string, any>();
const modelCache = new Map<string, any>();
const modelInflight = new Map<string, Promise<any>>();
...
export function createZedNativeAuthData(config: any = {}, options: any = {}) {
...
async function fetchJson(url: string, options: any): Promise<any> {
  const res = await proxyAwareFetch(url, options) as Response;
...
export async function fetchZedAuthenticatedUser(credentials: any, options: any = {}) {
...
export async function fetchZedLlmToken(credentials: any, options: any = {}) {
  const config = options.config || {};
...
export async function zedLlmFetch(credentials: any, path: string, options: any = {}): Promise<Response> {
  const config = options.config || {};
...
export async function resolveZedModels(credentials: any, options: any = {}) {
