...
export function parseSSEToOpenAIResponse(rawSSE, fallbackModel) {
  ...
  const message: Record<string, any> = { role: "assistant", content: contentParts.join("") || (toolCallMap.size > 0 ? null : "") };
...
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      const usage: Record<string, any> = jsonResponse.usage || {};
      ...
      if (sourceFormat === FORMATS.ANTIGRAVITY || sourceFormat === FORMATS.GEMINI || sourceFormat === FORMATS.GEMINI_CLI) {
        ...
        const message: Record<string, any> = { role: "assistant", content: textContent || (hasToolCalls ? null : "") };
  ...
  try {
    ...
    const parsed: any = parseSSEToOpenAIResponse(sseText, model);
