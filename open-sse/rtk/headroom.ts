...
async function callCompress(url, messages, model, timeoutMs, compressUserMessages, diagnostics) {
  ...
  const payload: Record<string, any> = { messages, model };
...
export async function compressWithHeadroom(body, { enabled, url, model, format, compressUserMessages, timeoutMs = DEFAULT_TIMEOUT_MS, diagnostics = null }: Record<string, any> = {}) {
  if (!enabled) {
