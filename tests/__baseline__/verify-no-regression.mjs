...
const nowFails = r.testResults.flatMap(f => {
  const normalizedName = String(f.name).replace(/\\/g, "/");
  const testsIndex = normalizedName.lastIndexOf("/tests/");
  const relativeName = testsIndex >= 0
    ? normalizedName.slice(testsIndex + 1)
    : normalizedName.replace(/^.*?(tests\/)/, "$1");
  return f.assertionResults.filter(a => a.status === "failed")
    .map(a => `${relativeName} :: ${a.fullName}`);
});
