...
describe("Codex reset credits", () => {
  ...
  it("returns normalized reset credit expiry details", async () => {
    ...
    const { getCodexRateLimitResetCredits } = await import("../../open-sse/services/usage/codex");
  ...
  it("GET refreshes OAuth credentials before returning reset credit details", async () => {
    ...
    const { GET } = await import("../../src/app/api/usage/[connectionId]/codex-reset-credits/route");
  ...
  it("POST returns 409 when there are no reset credits to consume", async () => {
    ...
    const { POST } = await import("../../src/app/api/usage/[connectionId]/codex-reset-credits/route");
