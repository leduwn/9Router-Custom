 ...
 */
export function parseQuotaData(provider: string, data: any) {
  if (!data || typeof data !== "object") return [];

  const normalizedQuotas: any[] = [];

  try {
    switch (provider.toLowerCase()) {
      case "github":
        if (data.quotas) {
          entriesOf(data.quotas).forEach(([name, quota]) => {
      ...
      case "antigravity":
        if (data.quotas) {
          entriesOf(data.quotas).forEach(([modelKey, quota]) => {
            normalizedQuotas.push({
      ...
      case "codex":
        if (data.quotas) {
          entriesOf(data.quotas).forEach(([quotaType, quota]) => {
            normalizedQuotas.push({
              name: quotaType,
              used: quota.used || 0,
              total: quota.total || 0,
