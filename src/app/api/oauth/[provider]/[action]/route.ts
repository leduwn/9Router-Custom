  ...
  try {
    ...
    if (action === "start-proxy") {
      ...
      if (provider === "zed") {
        ...
        const result = await startZedProxy(Number(searchParams.get("native_app_port") || ZED_HOSTED_CONFIG.defaultNativeAppPort));
        return NextResponse.json(result);
      }
  ...
  try {
    ...
    if (action === "exchange") {
      ...
      if (code && code.startsWith("eyJ") && code.includes(".")) {
        ...
        let directPayload: Record<string, any> = {};
        try {
        ...
        const providerSpecificData: Record<string, any> = { authMethod: "access_token" };
