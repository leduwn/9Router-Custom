 ...
 */
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta = null, idcConfig = null }) {
  ...
  const startProxyFlow = useCallback(async (providerId) => {
    ...
    const regBody: Record<string, any> = { state: authData.state };
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      if (oauthMeta) {
        Object.entries(oauthMeta).forEach(([k, v]) => { if (v) authorizeUrl.searchParams.set(k, String(v)); });
      }
