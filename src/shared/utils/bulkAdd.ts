 ...
 */
function parseLine(line, opts: Record<string, any> = {}): any {
 ...
 */
export function planBulkAdd(lines, existingNames, opts: Record<string, any> = {}) {
  const { isCloudflareAi = false } = opts;
  ...
  for (const raw of lines) {
    ...
    const entry: Record<string, any> = { name, apiKey: parsed.apiKey, skipped: false };
