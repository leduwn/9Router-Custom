...
function resolveBundledServerPath() {
  ...
  const fromRuntime = path.join(process.cwd(), ".runtime", "src", "mitm", "server.js");
...
function getPort443Owner(sudoPassword): Promise<{ pid: number; name: string } | null> {
  return new Promise((resolve) => {
    if (IS_WIN) {
      const psCmd = `powershell -NonInteractive -WindowStyle Hidden -Command "` +
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  const health: any = await pollMitmHealth(8000, MITM_PORT);
  if (!health) {
