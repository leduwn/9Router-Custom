  ...
  if (platform === "win32") {
    ...
    for (const procName of ["cloudflared", "tray_windows_release"]) {
      try {
        ...
        out.split("\n").forEach(l => {
          const pid = l.trim();
          if (pid && !Number.isNaN(Number(pid))) pids.push(Number(pid));
        });
      } catch { /* not running */ }
    }
  } else {
    try {
      ...
      output.split("\n").forEach(line => {
        ...
        if (isAppProcess) {
          ...
          const pid = parts[1];
          if (pid && !Number.isNaN(Number(pid)) && pid !== process.pid.toString()) pids.push(Number(pid));
        }
      });
    } catch { /* no processes */ }
  }
...
function resolveBundledUpdaterPath() {
  ...
  // Dev: generated runtime artifacts live under app/.runtime/.
  const fromRuntime = path.join(process.cwd(), ".runtime", "src", "lib", "updater", "updater.js");
  if (fs.existsSync(fromRuntime)) return fromRuntime;
...
export async function killAppProcesses() {
  ...
  pids.forEach(pid => {
    try {
      if (platform === "win32") {
        execSync(`taskkill /F /PID ${pid} 2>nul`, { stdio: "ignore", shell: process.env.ComSpec || "cmd.exe", windowsHide: true, timeout: 3000 });
      } else {
