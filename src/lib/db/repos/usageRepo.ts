...
import { incrementUsedTokensWithAdapter } from "./apiKeysRepo";
...
async function calculateCost(provider, model, tokens) {
  ...
  try {
    ...
    const { calculateCostFromTokens } = await import("open-sse/providers/pricing");
...
export async function getUsageStats(period = "all") {
  ...
  if (useDailySummary) {
    ...
    for (const dr of dayRows) {
      ...
      for (const [prov, p] of Object.entries(day.byProvider || {}) as [string, any][]) {
      ...
      for (const [mk, m] of Object.entries(day.byModel || {}) as [string, any][]) {
      ...
      for (const [connId, a] of Object.entries(day.byAccount || {}) as [string, any][]) {
      ...
      for (const [akKey, ak] of Object.entries(day.byApiKey || {}) as [string, any][]) {
      ...
      for (const [epKey, ep] of Object.entries(day.byEndpoint || {}) as [string, any][]) {
  ...
  stats.totalRequests = Object.values<any>(stats.byProvider).reduce((sum, p) => sum + (p.requests || 0), 0);
  return stats;
}
...
export async function appendRequestLog(_entry?: any) {}
