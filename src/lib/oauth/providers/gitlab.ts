...
const gitlab = {
  ...
  buildAuthUrl: (config, redirectUri, state, codeChallenge, meta: Record<string, any> = {}) => {
  ...
  exchangeToken: async (config, code, redirectUri, codeVerifier, state, meta: Record<string, any> = {}) => {
    const baseUrl = meta.baseUrl || config.defaultBaseUrl;
    const clientId = meta.clientId || "";
