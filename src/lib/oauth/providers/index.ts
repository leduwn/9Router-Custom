 ...
 */
export async function generateAuthData(providerName, redirectUri, meta = null) {
 ...
 * @param {object} [meta] - Provider-specific metadata (e.g. gitlab clientId/baseUrl)
 */
export async function exchangeTokens(providerName, code, redirectUri = null, codeVerifier = null, state = null, meta = null) {
  const provider = getProvider(providerName);
  const config = provider.prepareConfig
    ? await provider.prepareConfig(provider.config, meta || {})
    : provider.config;
 ...
 */
export async function requestDeviceCode(providerName, codeChallenge = null, options = null) {
  const provider = getProvider(providerName);
  if (provider.flowType !== "device_code") {
 ...
 */
export async function pollForToken(providerName, deviceCode, codeVerifier = null, extraData = null) {
  const provider = getProvider(providerName);
  if (provider.flowType !== "device_code") {
    throw new Error(`Provider ${providerName} does not support device code flow`);
  }
